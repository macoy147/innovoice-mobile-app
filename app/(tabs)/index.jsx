...
export default function SubmissionScreen() {
  ...
  useEffect(() => {
    if (params.editDraftId) {
      ...
      setShouldValidate(params.showValidation === 'true');
  ...
  }, [params.category, params.title, params.content, params.editDraftId, params._editTs, params.showValidation]);
  ...
  const handleSaveDraft = async (formData) => {
    try {
      ...
      if (editDraftId) {
        // Check if draft still exists before updating
        const existingDraft = await draftService.getDraft(editDraftId);
        if (existingDraft) {
          ...
          // Draft doesn't exist anymore, create a new one
          console.log('[Index] Draft not found, creating new draft instead');
      ...
      console.error('[Index] Error saving draft:', error);
