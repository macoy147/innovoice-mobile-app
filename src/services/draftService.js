...
class DraftService {
  ...
  async updateDraft(draftId, updates) {
    try {
      console.log('[DraftService] Updating draft:', draftId);
      const drafts = await StorageService.getDrafts();
      console.log('[DraftService] Total drafts found:', drafts.length);
      console.log('[DraftService] Draft IDs:', drafts.map(d => d.id));
      
      const draftIndex = drafts.findIndex(d => d.id === draftId);

      if (draftIndex === -1) {
        console.error('[DraftService] Draft not found with ID:', draftId);
        throw new Error('Draft not found');
      }

      console.log('[DraftService] Found draft at index:', draftIndex);
      const updatedDraft = {
        ...drafts[draftIndex],
        ...updates,
      ...
      console.log('[DraftService] Draft updated successfully');
      return updatedDraft;
    } catch (error) {
